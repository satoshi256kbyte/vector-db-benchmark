...
- [ ] 3. シェルスクリプトの基盤実装（引数パース・前提条件チェック・ユーティリティ）
  - [-] 3.1 `scripts/benchmark.sh` を作成し、基本構造を実装する
...
- [ ] 4. シェルスクリプトの ACU/OCU スケーリングとクリーンアップ実装
  - [~] 4.1 `scripts/benchmark.sh` に Aurora ACU スケーリング関数を追加する
  ...
  - [~] 4.2 `scripts/benchmark.sh` に OpenSearch OCU スケーリング関数を追加する
  ...
  - [~] 4.3 `scripts/benchmark.sh` に `cleanup` 関数と `trap cleanup EXIT` を実装する
    - `AURORA_SCALED_UP` / `OPENSEARCH_SCALED_UP` フラグで復元要否を判定
    - クリーンアップ内の AWS CLI 呼び出しは `2>/dev/null || true` でエラーを無視
    - _要件: 11.2, 11.3, 11.4_

- [ ] 5. シェルスクリプトのインデックス操作と ECS タスク実行
  - [~] 5.1 `scripts/benchmark.sh` に Aurora インデックス操作関数を追加する
  ...
  - [~] 5.2 `scripts/benchmark.sh` に OpenSearch インデックス操作関数を追加する
  ...
  - [~] 5.3 `scripts/benchmark.sh` に ECS タスク実行関数を追加する
...
- [ ] 6. シェルスクリプトのメトリクス収集と結果出力
  - [~] 6.1 `scripts/benchmark.sh` にレコード数取得関数を追加する
  ...
  - [~] 6.2 `scripts/benchmark.sh` にログ収集・メトリクス取得関数を追加する
  ...
  - [~] 6.3 `scripts/benchmark.sh` に結果 JSON 生成関数を追加する
...
- [ ] 7. シェルスクリプトのメインフロー統合
  - [~] 7.1 `scripts/benchmark.sh` に `run_benchmark_cycle` 関数と `main` 関数を実装する
...
- [ ] 8. チェックポイント - シェルスクリプト実装の確認
  - 全テスト pass を確認し、不明点があればユーザーに質問する

- [ ] 9. Python テスト（コスト算出・処理時間・結果 JSON）
  - [~] 9.1 `tests/scripts/test_duration_calc.py` にプロパティテストを作成する
    - **プロパティ 3: 処理時間算出の正確性**
    - **検証対象: 要件 5.6**
  - [~] 9.2 `tests/scripts/test_cost_calculation.py` にプロパティテストを作成する
    - **プロパティ 4: Fargate 概算コスト算出の正確性**
    - **検証対象: 要件 8.3**
  - [~] 9.3 `tests/scripts/test_result_json.py` にプロパティテストを作成する
    - **プロパティ 5: 結果 JSON の必須フィールド完全性**
    - **検証対象: 要件 9.2, 9.4**

- [ ] 10. シェルスクリプトテスト（bats-core）
  - [~] 10.1 `tests/scripts/test_benchmark.bats` にコマンドライン引数パーステストを作成する
    - **プロパティ 6: コマンドライン引数パースとデフォルト値**
    - **検証対象: 要件 10.1, 10.2, 10.3, 10.4, 10.5, 10.6, 10.7, 10.8**
  - [~] 10.2 `tests/scripts/test_benchmark.bats` に前提条件チェック・ヘルプ表示・エラー継続のユニットテストを作成する
