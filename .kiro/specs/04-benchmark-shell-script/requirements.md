...
本 Spec ではインデックス操作と ACU/OCU スケーリングをシェルスクリプト側に移管し、
ECS タスクの役割をデータ投入のみに簡素化する。

各DBのベンチマークサイクルは以下の流れで実行する:
1. ACU 拡張（Aurora のみ）/ OCU スケーリング（OpenSearch のみ）
2. 投入前レコード数記録
3. インデックス削除（シェルスクリプトが AWS CLI/psql 経由で実行）
4. 開始時刻記録
5. ECS タスク起動（データ投入のみ）
6. ECS タスク完了待機
7. 終了時刻記録
8. インデックス作成（シェルスクリプトが AWS CLI/psql 経由で実行）
9. 投入後レコード数記録
10. 処理時間記録
11. ログ収集
12. ACU/コスト記録
13. ACU 縮小（Aurora のみ）/ OCU 復元（OpenSearch のみ）
...
### 要件 2: Aurora Serverless v2 ACU スケーリング（シェルスクリプトから AWS CLI 経由）

**ユーザーストーリー:** 検証担当者として、
ローカル PC からシェルスクリプトで Aurora Serverless v2 の ACU を
AWS CLI 経由で拡張・縮小したい。
投入パフォーマンスを最大化しつつ、
未使用時のコストを最小化するためである。
...
1. WHEN ベンチマークスクリプトが Aurora のデータ投入を開始する前に、
   THE ベンチマークスクリプト SHALL `aws rds modify-db-cluster` コマンドで
   Aurora Serverless v2 クラスターの最小 ACU を指定値（デフォルト 8）に拡張する
2. WHEN Aurora のデータ投入が完了した後、
   THE ベンチマークスクリプト SHALL `aws rds modify-db-cluster` コマンドで
   Aurora Serverless v2 クラスターの最小 ACU を元の値（デフォルト 0）に戻す
3. THE ベンチマークスクリプト SHALL ACU 変更後、
   クラスターのステータスが `available` になるまで
   ポーリング（30秒間隔、最大20分）で待機する
4. THE ベンチマークスクリプト SHALL ACU 拡張前と縮小後の
   ACU 設定値をログに記録する
5. IF ACU 変更が失敗した場合、
   THEN THE ベンチマークスクリプト SHALL エラーメッセージを出力し
   処理を中断する

### 要件 3: OpenSearch Serverless OCU スケーリング（シェルスクリプトから AWS CLI 経由）

**ユーザーストーリー:** 検証担当者として、
ローカル PC からシェルスクリプトで OpenSearch Serverless の OCU 上限を
AWS CLI 経由で拡張・復元したい。
データ投入時のスループットを確保しつつ、
未使用時のコストを最小化するためである。
...
1. WHEN ベンチマークスクリプトが OpenSearch のデータ投入を開始する前に、
   THE ベンチマークスクリプト SHALL
   `aws opensearchserverless update-account-settings` コマンドで
   インデックス用 OCU の上限を指定値（デフォルト 10）に拡張する
2. WHEN OpenSearch のデータ投入が完了した後、
   THE ベンチマークスクリプト SHALL
   `aws opensearchserverless update-account-settings` コマンドで
   インデックス用 OCU の上限を元の値（デフォルト 2）に戻す
3. THE ベンチマークスクリプト SHALL OCU 変更前後の設定値をログに記録する
4. IF OCU 変更が失敗した場合、
   THEN THE ベンチマークスクリプト SHALL エラーメッセージを出力し
   処理を中断する

### 要件 4: インデックス操作（シェルスクリプトから外部実行）

**ユーザーストーリー:** 検証担当者として、
シェルスクリプトからインデックスの削除と作成を行いたい。
...
1. WHEN ベンチマークスクリプトが Aurora のデータ投入を開始する前に、
   THE ベンチマークスクリプト SHALL Secrets Manager から Aurora の認証情報を取得し、
   psql コマンドで HNSW インデックス（embeddings_hnsw_idx）の DROP と
   テーブルデータの TRUNCATE を実行する
2. WHEN Aurora のデータ投入が完了した後、
   THE ベンチマークスクリプト SHALL psql コマンドで
   HNSW インデックス（embeddings_hnsw_idx）を再作成する
3. WHEN ベンチマークスクリプトが OpenSearch のデータ投入を開始する前に、
   THE ベンチマークスクリプト SHALL OpenSearch Serverless の
   インデックス（embeddings）を削除する
4. WHEN OpenSearch のデータ投入が完了した後、
   THE ベンチマークスクリプト SHALL OpenSearch Serverless の
   インデックス（embeddings）を HNSW マッピング付きで再作成する
5. THE ベンチマークスクリプト SHALL S3 Vectors に対しては
   インデックス操作を行わない（S3 Vectors にはユーザー制御可能なインデックスがないため）
6. THE ベンチマークスクリプト SHALL インデックス削除・作成の各操作結果をログに記録する
7. IF インデックス操作が失敗した場合、
   THEN THE ベンチマークスクリプト SHALL エラーメッセージを出力し
   該当DBの処理を中断する

**補足: OpenSearch Serverless の VPC アクセス制限について**

OpenSearch Serverless コレクション（vdbbench-dev-oss-vector）は
VPC ネットワークポリシーにより VPC 内からのみアクセス可能である。
ローカル PC から直接 OpenSearch のインデックス操作 API を呼び出すことはできない。
以下のいずれかの方法で対応する:

- 方法 A: ECS タスクに OpenSearch のインデックス操作のみを行う
  別モード（環境変数等）を追加し、シェルスクリプトから ECS タスク経由で実行する
- 方法 B: Lambda 関数を作成し、シェルスクリプトから
  `aws lambda invoke` で呼び出してインデックス操作を実行する
- 方法 C: ECS タスクが OpenSearch に対してのみ
  インデックス操作を引き続き行う（TARGET_DB=opensearch の場合のみ例外）
...
3. THE ベンチマークスクリプト SHALL Aurora のレコード数を
   Secrets Manager から取得した認証情報を使用して
   psql 経由の SQL クエリ（SELECT COUNT）で取得する
4. THE ベンチマークスクリプト SHALL OpenSearch のレコード数を
   VPC 内からアクセス可能な方法（要件 4 の補足と同様の手段）で取得する
5. THE ベンチマークスクリプト SHALL S3 Vectors のレコード数を
   `aws s3vectors list-vectors` API で取得する

### 要件 7: CloudWatch Logs からのログ収集
...
### 要件 8: コスト・ACU/OCU メトリクス記録

**ユーザーストーリー:** 検証担当者として、
ベンチマーク実行中の ACU/OCU 使用量とコスト関連情報を記録したい。
...
6. THE ベンチマークスクリプト SHALL AWS リージョンを
   ...
   指定可能とする
7. THE ベンチマークスクリプト SHALL ACU 拡張時の最小 ACU 値を
   コマンドライン引数 `--aurora-min-acu`（デフォルト 8）で
   指定可能とする
8. THE ベンチマークスクリプト SHALL OCU 拡張時の上限値を
   コマンドライン引数 `--opensearch-max-ocu`（デフォルト 10）で
   指定可能とする
9. THE ベンチマークスクリプト SHALL `--help` オプションで
   利用可能な全引数とデフォルト値を表示する

### 要件 11: エラーハンドリングと堅牢性

**ユーザーストーリー:** 検証担当者として、
スクリプトがエラー発生時にも適切に処理を継続し、
ACU/OCU を元に戻すことを保証したい。
コスト超過や環境の不整合を防ぐためである。
...
2. THE ベンチマークスクリプト SHALL trap を使用して
   スクリプトの異常終了時に Aurora の ACU と
   OpenSearch の OCU を元の値に戻す
   クリーンアップ処理を実行する
...
5. THE ベンチマークスクリプト SHALL 実行開始時に
   必要な前提条件（aws CLI、psql、jq コマンドの存在、
   AWS 認証情報の有効性）を確認する
...
2. THE ベンチマークスクリプト SHALL 各DBのベンチマークサイクルで
   以下の手順を順次実行する:
   ACU/OCU 拡張（該当DBのみ）→ 投入前レコード数記録 →
   インデックス削除（シェルスクリプトが実行）→
   開始時刻記録 → ECS タスク起動（データ投入のみ）→
   ECS タスク完了待機 → 終了時刻記録 →
   インデックス作成（シェルスクリプトが実行）→
   投入後レコード数記録 → ログ収集 → ACU/コスト記録 →
   ACU/OCU 縮小（該当DBのみ）
