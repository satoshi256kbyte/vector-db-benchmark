...
Aurora pgvector ではインデックスを一旦削除してからデータを投入し、
投入完了後にインデックスを再作成する戦略を採用する。
OpenSearch Serverless はインデックスの削除・再作成が有効でないため、
既存インデックスに対して Bulk API で一括投入する。
...
### 要件 3: Aurora インデックス戦略（削除→投入→再作成）

**ユーザーストーリー:** 検証担当者として、
Aurora pgvector への大量データ投入時にインデックスを一旦削除してから投入し、
投入後にインデックスを再作成したい。
...
3. THE 投入ECSタスク SHALL OpenSearch Serverless に対しては
   インデックス削除・再作成を行わず、
   既存インデックスに Bulk API で一括投入する
4. THE 投入ECSタスク SHALL S3 Vectorsに対しては
   インデックス削除・再作成を行わず、そのままデータを投入する
5. THE 投入ECSタスク SHALL インデックス削除、データ投入、
   インデックス再作成の各フェーズの所要時間を
   個別に計測しログに出力する
6. IF インデックス削除またはインデックス再作成が失敗した場合、
...
1. THE 投入ECSタスク SHALL 各DBについて
   データ投入時間、合計時間を記録する
   （Aurora はインデックス削除時間、インデックス再作成時間も記録する）
